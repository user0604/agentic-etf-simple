...
function smartFormatDetail(detail, agent) {
  ...
  if (agent === 'B') {
    ...
    if (Array.isArray(tasks) && tasks.length > 0 && !Array.isArray(holdings)) return formatTasks(detail);
    if (Array.isArray(holdings) && holdings.length > 0) return formatPortfolio(detail);
    if (detail.tasks && Array.isArray(detail.tasks)) return formatTasks(detail);
  }
  ...
  if (agent && agent.startsWith('X')) {
    if (detail.candidates && Array.isArray(detail.candidates)) return formatCandidates(detail);
    if (detail.plan && typeof detail.plan === 'object' && !Array.isArray(detail.plan)) return formatResearchPlan(detail.plan);
    if (detail.topic || detail.candidate_screening_criteria || detail.approach) return formatResearchPlan(detail);
  }
...
function formatGenericJson(detail) {
  ...
  for (const [key, value] of Object.entries(obj)) {
    ...
    if (Array.isArray(value)) {
      ...
      lines.push({ label, value: JSON.stringify(value, null, 1), raw: value });
    } else {
...
function DetailCollapsible({ detail, agent }) {
  ...
  return (
    <div style={{ marginLeft: 24, marginTop: 4 }}>
      ...
      {open && (
        <div style={{ marginTop: 6, padding: 10, background: '#0d1117', borderRadius: 6, border: '1px solid #21262d', fontSize: 12, lineHeight: 1.6 }}>
          {formatted}
        </div>
      )}
    </div>
...
export default function ActivityFeed({ events, agentDisplayFn }) {
  ...
  for (const ev of events) {
    ...
    displayEvents.push({ ...ev, _detail: ownDetail || pendingDetail || null, _detailAgent: ev.agent });
    pendingDetail = null;
        ...
        {displayEvents.map((ev, i) => (
          <div key={i} style={{ marginBottom: 8 }}>
            <div style={{
              ...
              <span style={{ color: iconColor(ev), flexShrink: 0 }}>[{icon(ev)}]</span>
              <span style={{ fontWeight: 600, color: '#d2a8ff', flexShrink: 0 }}>{displayFn(ev.agent)}:</span>
              <span>{formatEventMessage(ev)}</span>
            </div>
            {ev._detail && <DetailCollapsible detail={ev._detail} agent={ev._detailAgent || ev.agent} />}
          </div>
        ))}
        <div ref={bottomRef} />
      </div>
